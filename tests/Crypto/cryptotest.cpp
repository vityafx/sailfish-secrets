...
//        dcr.setManager(&m_sm);
//        dcr.setCollectionName(testCollection.name);
//        dcr.setStoragePluginName(testCollection.storagePlugin);
//        dcr.setUserInteractionMode(testCollection.userInteractionMode);
//        dcr.startRequest();
//        WAIT_FOR_REQUEST(dcr);  // ignore failures, collection may not have been created due to test failures
    }
}
                                   ...
                                   const QByteArray &plaintext)
{
    ...
    QTest::newRow("AES ECB 128-bit")
            ...
            << QByteArray() << plaintextData << initVector;
    ...
    QTest::newRow("AES GCM 128-bit")
            ...
            << createTestKey(128, CryptoManager::AlgorithmAes, keyOrigin, operations, keyIdentifier)
            << authData16 << plaintextData << initVector;
    ...
    QTest::newRow("RSA 512-bit (no padding)")
            ...
            << QByteArray() << generateRsaPlaintext(CryptoManager::EncryptionPaddingNone, 512) << QByteArray();
    QTest::newRow("RSA 512-bit (PKCS1 padding")
            << plugins << CryptoManager::BlockModeUnknown << CryptoManager::EncryptionPaddingRsaPkcs1
            << createTestKey(512, CryptoManager::AlgorithmRsa, keyOrigin, operations, keyIdentifier)
            << QByteArray() << generateRsaPlaintext(CryptoManager::EncryptionPaddingRsaPkcs1, 512) << QByteArray();
    QTest::newRow("RSA 512-bit (OAEP padding)")
            << plugins << CryptoManager::BlockModeUnknown << CryptoManager::EncryptionPaddingRsaOaep
            << createTestKey(512, CryptoManager::AlgorithmRsa, keyOrigin, operations, keyIdentifier)
            << QByteArray() << generateRsaPlaintext(CryptoManager::EncryptionPaddingRsaOaep, 512) << QByteArray();
    QTest::newRow("RSA 1024-bit (no padding)")
            << plugins << CryptoManager::BlockModeUnknown << CryptoManager::EncryptionPaddingNone
            << createTestKey(1024, CryptoManager::AlgorithmRsa, keyOrigin, operations, keyIdentifier)
            << QByteArray() << generateRsaPlaintext(CryptoManager::EncryptionPaddingNone, 1024) << QByteArray();
    QTest::newRow("RSA 1024-bit (PKCS1 padding")
            << plugins << CryptoManager::BlockModeUnknown << CryptoManager::EncryptionPaddingRsaPkcs1
            << createTestKey(1024, CryptoManager::AlgorithmRsa, keyOrigin, operations, keyIdentifier)
            << QByteArray() << generateRsaPlaintext(CryptoManager::EncryptionPaddingRsaPkcs1, 1024) << QByteArray();
    QTest::newRow("RSA 1024-bit (OAEP padding)")
            << plugins << CryptoManager::BlockModeUnknown << CryptoManager::EncryptionPaddingRsaOaep
            << createTestKey(1024, CryptoManager::AlgorithmRsa, keyOrigin, operations, keyIdentifier)
            << QByteArray() << generateRsaPlaintext(CryptoManager::EncryptionPaddingRsaOaep, 1024) << QByteArray();
